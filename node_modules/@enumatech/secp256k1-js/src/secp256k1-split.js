(function () {
    ...
    // This works but exposes d2 to P1
    function ecsign_candidate1 (d1, d2, z) {
        const dInv = invmod(d2, N)
        const e1 = mulmod(dInv, z, N)
        // Party 1 signs e/d2
        const sig1 = ecsign(d1, e1)
        const r = sig1.r
        const s = mulmod(uint256(sig1.s,16), d2, N)
        return {r: toHex(r), s: toHex(s), v: sig1.v}
    }

    function ecsign2 (d1, d2, z) {
        const dInv = invmod(d2, N)
        const e1 = mulmod(dInv, z, N)
        const k2 = rnd(P)
        const R2 = mulG(k2)

        // Party 1 signs e.r1/r.d2
        let sig1, Rk, rr
        while (true) {
            const k = rnd(P)
            const R = mulG(k)
            if (R[0] == 0) continue
            // feedback
            Rk = JtoA(ecmul(AtoJ(R2[0],R2[1]), k))
            rr = mulmod(invmod(Rk[0], N), R[0], N)
            const z = mulmod(e1, rr, N)
            // proceed with normal ECDSA
            const s = mulmod(invmod(k, N), addmod(z, mulmod(R[0], d1, N), N), N)
            if (s == 0) continue
            //FIXME: why do I need this
            if (s.testn(255)) continue
            sig1 = {r: toHex(R[0]), s: toHex(s), v: R[1].testn(0) ? 1 : 0}
            break
        }

        // Party 2 signs
        const s1 = uint256(sig1.s, 16)
        const oorr = invmod(rr, N)
        const s2 = mulmod(s1, mulmod(mulmod(d2, oorr, N), invmod(k2, N), N), N)
        return {r: toHex(Rk[0]), s: toHex(s2), v: Rk[1].testn(0) ? 1 : 0}
    }
